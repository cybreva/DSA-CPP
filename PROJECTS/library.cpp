...
time_t string_to_time(string datetime)
{
    tm tm_obj = {};

    stringstream ss(datetime);

    ss >> get_time(&tm_obj, "%d-%m-%Y %H:%M:%S");

    return mktime(&tm_obj);
}
long calculate_fine(time_t due_time)
{
    time_t now = time(0);
    ...
    long seconds_late = now - due_time;

    long days_late = seconds_late / (24 * 60 * 60);
...
void return_book()
{
    ...
    while(getline(file, line))
    {
        ...
        {
            time_t due_time = string_to_time(due);

            long fine = calculate_fine(due_time);
